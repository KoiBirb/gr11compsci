...
public class Main {
     ...
     */
    public static boolean checkWin(String guess, int[] code){
        for(int i = 0; i < guess.length(); i++){
            if(Character.getNumericValue(guess.charAt(i)) != code[i]){
                return false;
            }
        }
        return true;
     ...
     * @param code int[] code to guess
     */
    public static void getClues(String guess, int[] code){
        int pegsCorrect = 0, coloursCorrect = 0;
        for(int i = 0; i < guess.length(); i++){
            if(Character.getNumericValue(guess.charAt(i)) == code[i]) pegsCorrect++;
            for (int k : code)
                if (Character.getNumericValue(guess.charAt(i)) == k) coloursCorrect++;
        }
